...
__global__ void benchmark_alt(int *d_X, uint64_t *d_startClk,
  ...
  for (int i = 0; i < ITERATIONS; i++) {
    // printf("addr = %d\n", addr);
    //  assembly mma
    asm volatile("ldmatrix.sync.aligned.x2.m8n8.shared.b16 {%0, %1}, [%0];"
                 : "+r"(x), "=r"(y));
  }
  // printf("x = %d\n", x);
  // for (int i = 0; i < ITERATIONS; i++) {
...
int main() {
  ...
  long bytes =
      8 * 8 * 2 * ITERATIONS * (THREADS_PER_BLOCK / 32) * NUM_BLOCKS * 2;
  // 8 rows of 8 elements each 2 bytes, 2 loads
