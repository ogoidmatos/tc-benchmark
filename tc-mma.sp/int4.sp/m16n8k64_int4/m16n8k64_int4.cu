...
__global__ void benchmark_alt(int *d_A, int *d_B, int *d_C,
  ...
  for (int i = 0; i < ITERATIONS; i++) {
    ...
    asm volatile(
        ...
        : "r"(fragsA[0]), "r"(fragsA[1]), "r"(fragsB[0]), "r"(fragsB[1]),
          "r"(meta));
    //__syncwarp();
  }
  // stop timing
...
int main() {
  ...
  std::cout << "mma.sp.sync.aligned.m16n8k64.row.col.s32.s4.s4.s32  latency "
            << (float)total_clk / (float)ITERATIONS << " cycles\n";
  std::cout << "mma.sp.sync.aligned.m16n8k64.row.col.s32.s4.s4.s32  FMA Count "
            << fma << "\n";
